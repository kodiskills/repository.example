...
from scheduleParser import parse_schedule_xmltv
from strings import *
...
class Program(object):
    ...
    def __repr__(self):
        return 'Program(channel=%s, title=%s, startDate=%s, endDate=%s, description=%s, imageLarge=%s, ' \
               'imageSmall=%s, episode=%s, season=%s, is_movie=%s, language=%s, category=%s)' % (
                   ...
                   self.is_movie, self.language, self.category)
...
class Database(object):
    ...
    def __init__(self, show_popups=True):
        self.conn = None
        self.eventQueue = list()
        self.event = threading.Event()
        self.eventResults = dict()

        self.source = instantiateSource(show_popups)
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[plugin.video.sportsaccess] Database is locked, bailing out...',
                             xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[plugin.video.sportsaccess] Database is locked, retrying...',
                             xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[plugin.video.sportsaccess] Database is broken and unlink() failed',
                             xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback,
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[plugin.video.sportsaccess] Updating caches...', xbmc.LOGDEBUG)
            ...
            all_data = []
            for item in self.source.getChannelDataFromExternal(date, progress_callback):
                all_data.append(item)

            for item in self.source.getProgrammeDataFromExternal(date, progress_callback):
                all_data.append(item)
            ...
            for item in all_data:
                ...
                if isinstance(item, Channel):
                    ...
                    c.execute(
                        ...
                        [channel.id, channel.title, channel.logo, channel.streamUrl,
                         channel.visible, channel.weight,
                    ...
                    if not c.rowcount:
                        c.execute(
                            ...
                            [channel.title, channel.logo, channel.streamUrl, channel.weight,
                             channel.visible,
                    ...
                    xbmc.log('[%s] INSERT: channel: %s, start: %s' % (ADDON_ID, channel,
                                                                      program.startDate),
                             xbmc.LOGDEBUG)
                    try:
                        c.execute(
                            'INSERT INTO programs(channel, title, start_date, end_date, description, '
                            'image_large, image_small, season, episode, is_movie, language, source, '
                            'updates_id, category) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            [channel, program.title, program.startDate, program.endDate,
                             ...
                             program.is_movie,
                             program.language, self.source.KEY, updatesId, program.category])
    ...
    def search(self, query, max_recent=1):
        result = self._invokeAndBlockForResult(self._search, query, max_recent)

        return result

    def _search(self, query, max_recent):
        xbmc.log("[%s] Starting search with query '%s'" % (ADDON_ID, str(query)), xbmc.LOGDEBUG)

        now = datetime.datetime.now()
        recent = now - datetime.timedelta(hours=max_recent)
        programList = {'ended': list(), 'now': list(), 'future': list()}

        base_sql = 'SELECT p.*, c.id as cid, c.title as c_title, c.stream_url FROM programs p, channels c ' \
                   'WHERE cid = p.channel'

        for item in query:
            base_sql += ' AND p.title LIKE "%' + item + '%"'

        c = self.conn.cursor()
        c.execute(base_sql + ' AND p.end_date < ? AND p.end_date >= ? ORDER BY p.start_date, cid',
                  [now, recent])
        for row in c:
            channel = Channel(row['channel'], row['c_title'], streamUrl=row['stream_url'])
            program = Program(channel, row['title'], row['start_date'],
                              ...
                              row['category'])
            programList['ended'].append(program)
        xbmc.log("[%s] Got 'ended' programs" % ADDON_ID, xbmc.LOGDEBUG)

        c.execute(base_sql + ' AND p.start_date <= ? AND p.end_date > ? ORDER BY p.start_date, cid', [now, now])
        for row in c:
            channel = Channel(row['channel'], row['c_title'], streamUrl=row['stream_url'])
            program = Program(channel, row['title'], row['start_date'],
                              row['end_date'],
                              ...
                              row['season'], row['episode'], row['is_movie'], row['language'],
                              row['category'])
            programList['now'].append(program)
        xbmc.log("[%s] Got 'now' programs" % ADDON_ID, xbmc.LOGDEBUG)

        c.execute(base_sql + ' AND p.start_date > ? ORDER BY p.start_date, cid', [now])
        for row in c:
            channel = Channel(row['channel'], row['c_title'], streamUrl=row['stream_url'])
            program = Program(channel, row['title'], row['start_date'],
                              row['end_date'],
                              ...
                              row['season'], row['episode'], row['is_movie'], row['language'],
                              row['category'])
            programList['future'].append(program)
        xbmc.log("[%s] Got 'future' programs" % ADDON_ID, xbmc.LOGDEBUG)
    ...
    def _saveChannelList(self, channelList):
        ...
        for idx, channel in enumerate(channelList):
            c.execute(
                'INSERT OR IGNORE INTO channels(id, title, logo, stream_url, visible, weight, source) VALUES(?, ?, ?, ?, ?, (CASE ? WHEN -1 THEN (SELECT COALESCE(MAX(weight)+1, 0) FROM channels WHERE source=?) ELSE ? END), ?)',
                [channel.id, channel.title, channel.logo, channel.streamUrl, channel.visible,
                 channel.weight,
                 self.source.KEY, channel.weight, self.source.KEY])
            if not c.rowcount:
                c.execute(
                    'UPDATE channels SET title=?, logo=?, stream_url=?, visible=?, weight=(CASE ? WHEN -1 THEN weight ELSE ? END) WHERE id=? AND source=?',
                    [channel.title, channel.logo, channel.streamUrl, channel.visible,
    ...
    def _getCurrentProgram(self, channel):
        ...
        if row:
            program = Program(channel, row['title'], row['start_date'], row['end_date'],
                              ...
                              row['episode'],
                              row['is_movie'], row['language'], row['category'])
        c.close()
    ...
    def _createTables(self):
        ...
        try:
            ...
            if version < [1, 3, 2]:
                ...
                c.execute('DROP TABLE programs')
                c.execute(
                    'CREATE TABLE programs(channel TEXT, title TEXT, start_date TIMESTAMP, end_date TIMESTAMP, description TEXT, image_large TEXT, image_small TEXT, season TEXT, episode TEXT, is_movie TEXT, language TEXT, category TEXT, source TEXT, updates_id INTEGER, FOREIGN KEY(channel, source) REFERENCES channels(id, source) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED, FOREIGN KEY(updates_id) REFERENCES updates(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED)')
                c.execute(
                    'CREATE INDEX program_list_idx ON programs(source, channel, start_date, end_date)')
                c.execute('CREATE INDEX start_date_idx ON programs(start_date)')
                c.execute('CREATE INDEX end_date_idx ON programs(end_date)')
    ...
    def _getNotifications(self, daysLimit):
        ...
        c = self.conn.cursor()
        c.execute(
            "SELECT DISTINCT n.channel, c.title, p.title, p.start_date "
            "FROM notifications n, channels c, programs p "
            "WHERE n.channel = c.id "
            "AND p.channel = c.id "
            "AND n.program_title = p.title "
            "AND n.source=? "
            "AND p.start_date >= ? "
            "AND p.end_date <= ?",
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', ADDON_ID))
    KEY = 'xmltv'
    GUIDE_DATA = "1"
    SCHEDULE_DATA = "2"
    CHANNEL_DATA = "3"

    def __init__(self, addon, show_popups=True):

        self.needReset = False
        self.fetchError = False
        self.xmltvInterval = -1
        ...
        self.xmltvChFile = self.update_local_file(XMLTVSource.CHANNEL_DATA, addon, FileFetcher.INTERVAL_12)
        self.xmltvFile = self.update_local_file(XMLTVSource.GUIDE_DATA, addon, FileFetcher.INTERVAL_24)
        self.xmltvScheduleFile = self.update_local_file(XMLTVSource.SCHEDULE_DATA, addon, FileFetcher.INTERVAL_2)

        if not self.xmltvFile or not xbmcvfs.exists(self.xmltvFile):
            raise SourceNotConfiguredException()

    def update_local_file(self, name, addon, interval):
        path = os.path.join(XMLTVSource.PLUGIN_DATA, name)
        fetcher = FileFetcher(name, addon, self.show_popups, interval=interval)
        retVal = fetcher.fetch_file()
        if retVal == fetcher.FETCH_OK:
            self.needReset = True
        elif retVal == fetcher.FETCH_ERROR and self.show_popups:
            xbmcgui.Dialog().ok(strings(FETCH_ERROR_TITLE), strings(FETCH_ERROR_LINE1),
                                strings(FETCH_ERROR_LINE2))

        return path

    def getChannelDataFromExternal(self, date, progress_callback=None):

        f = FileWrapper(self.xmltvChFile)
        context = ElementTree.iterparse(f, events=("start", "end"))
        size = f.size

        return self.parseXMLTV(context, f, size, self.logoFolder, progress_callback)

    def getProgrammeDataFromExternal(self, date, progress_callback=None):
        f = FileWrapper(self.xmltvFile)
        context = ElementTree.iterparse(f, events=("start", "end"))
        size = f.size

        return self.parseXMLTV(context, f, size, self.logoFolder, progress_callback, True)
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                t_tmp = datetime.datetime.fromtimestamp(
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback, only_programmes=False):
        ...
        crypto_programmes = parse_schedule_xmltv(self.xmltvScheduleFile)
        for c_idx, item in enumerate(crypto_programmes):
            item['startDate'] = self.parseXMLTVDate(item['startDate'])
            item['endDate'] = self.parseXMLTVDate(item['endDate'])
        ...
        for event, elem in context:
            if event == "end":
                ...
                if elem.tag == "programme":
                    # Make ID safe to use as ' can cause crashes!
                    ...
                    endDate = self.parseXMLTVDate(elem.get('stop'))
                    prg_title = elem.findtext('title')
                    
                    xbmc.log("[%s] Adding programme: ch: %s, title: %s, start: %s, end: %s" %
                             (ADDON_ID, channel, prg_title.encode('utf8', 'replace'), startDate, endDate), xbmc.LOGDEBUG)
                    result = Program(channel, prg_title, startDate, endDate,
                                     description, imageSmall=icon, season=season, episode=episode,
                                     is_movie=is_movie, language=language)

                    delete_result = False
                    for c_idx, item in enumerate(crypto_programmes):
                        if item["channel"] == channel and startDate < item["endDate"] <= endDate:
                            xbmc.log("[%s] Adjusting programme: ch: %s, title: %s, old_start: %s, new_start: %s" %
                                     (ADDON_ID, channel, prg_title.encode('utf8', 'replace'), startDate, item['endDate']), xbmc.LOGDEBUG)
                            result.startDate = item['endDate']

                        if item["channel"] == channel and startDate <= item["startDate"] < endDate:
                            xbmc.log("[%s] Adding override programme: ch: %s, title: %s, start: %s, end: %s" %
                                     (ADDON_ID, channel, item["title"].encode('utf8', 'replace'), item["startDate"], item["endDate"]), xbmc.LOGDEBUG)
                            result2 = Program(channel, item["title"], item["startDate"],
                                              item["endDate"],
                                              'Live %s Event' % cats.get_label(item['category']),
                                              imageSmall=None, season=None, episode=None,
                                              is_movie=False, language='',
                                              category=item['category'])
                            xbmc.log("[%s] Adjusting programme: ch: %s, title: %s, old_end: %s, new_end: %s" %
                                     (ADDON_ID, channel, prg_title.encode('utf8', 'replace'), endDate, item['startDate']), xbmc.LOGDEBUG)
                            result.endDate = item['startDate']
                            used_programmes.append(c_idx)

                        if item['channel'] == channel and \
                                        startDate >= item['startDate'] and \
                                        endDate <= item['endDate']:
                            xbmc.log("[%s] Removing programme: ch: %s, title: %s, start: %s, end: %s" %
                                     (ADDON_ID, channel, prg_title.encode('utf8', 'replace'), startDate, endDate), xbmc.LOGDEBUG)
                            delete_result = True
                ...
                elif not only_programmes and elem.tag == "channel":
                    # Make ID safe to use as ' can cause crashes!
                    ...
                    if streamElement is not None:
                        ...
                        streamUrl = urllib.quote('plugin://' + ADDON_ID + '/?url=' + cid +
                                                 '&mode=565&name=' + title, safe='?&=:/')
                    ...
                    if visible == "0":
                        ...
                        visible = True
                    result = Channel(cid, title, icon, streamUrl, visible)
        ...
        if only_programmes and len(used_programmes) != len(crypto_programmes):
            # We have programmes that weren't added yet!
            for idx, item in enumerate(crypto_programmes):
                if idx not in used_programmes:
                    yield Program(item["channel"], item["title"], item["startDate"],
                                  item["endDate"],
                                  'Live %s Event' % cats.get_label(item['category']),
                                  imageSmall=None, season=None, episode=None, is_movie=False,
                                  language='', category=item['category'])
...
def instantiateSource(show_popups=True):
    return XMLTVSource(ADDON, show_popups)
